    ...
    <form @submit.prevent="submit">
      ...
      <div class="form-card">
        ...
        <div class="form-group">
          ...
          <InputText id="add-url" v-model="form.url" required placeholder="https://api.example.com/health" class="w-full" />
        </div>
      ...
      <div class="form-actions">
        <Button type="button" label="Cancel" severity="secondary" outlined @click="router.push('/dashboard')" />
...
async function submit() {
  ...
  try {
    ...
    if (err?.response?.status === 402) {
      toast.add({ severity: 'warn', summary: 'Plan limit reached', detail: msg ?? 'Upgrade to add more monitors', life: 6000 })
    } else {
      toast.add({ severity: 'error', summary: 'Failed to add monitor', detail: msg ?? 'Something went wrong', life: 6000 })
    }
  } finally {
